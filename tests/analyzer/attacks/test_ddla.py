...
from cw_ml_plugin.analyzer.attacks.ddla import DDLA
from cw_ml_plugin.analyzer.attacks.algorithms import MSB
...
def test_algorithm(base_class: DDLA) -> None:
    assert type(base_class.algorithm) == cwma.attacks.DDLA_Algorithm
    
    base_class.algorithm = CPAProgressive # type: ignore
    assert type(base_class.algorithm) == CPAProgressive
    
def test_point_range(base_class: DDLA) -> None:
...
def test_trace_range(base_class:DDLA) -> None:
...
def test_epoch(base_class:DDLA) -> None:
...
def test_batch_size(base_class:DDLA) -> None:
...
def test_byte_range(base_class:DDLA) -> None:
    assert base_class.byte_range == range(16)
    
    with pytest.raises(TypeError):
        base_class.byte_range = 10 # type: ignore
        
    base_class.byte_range = [1,2,3]
    assert base_class.byte_range == [1,2,3]

def test_key_range(base_class:DDLA) -> None:
    assert base_class.key_range == range(256)
    
    with pytest.raises(TypeError):
        base_class.key_range = 10 # type: ignore
        
    base_class.key_range = [1,2,3]
    assert base_class.key_range == [1,2,3]

def test_sort_param(base_class:DDLA) -> None:
    assert base_class.sort_param == 'senstivity'
    
    with pytest.raises(TypeError):
        base_class.sort_param = 'hello' # type: ignore
        
    base_class.sort_param = 'accuracy'
    assert base_class.sort_param == 'accuracy'

def test_max_parallel_jobs(base_class:DDLA) -> None:
    assert base_class.max_parallel_jobs == 1
    
    with pytest.raises(TypeError):
        base_class.max_parallel_jobs = [] # type: ignore
        
    base_class.max_parallel_jobs = 5
    assert base_class.max_parallel_jobs == 5