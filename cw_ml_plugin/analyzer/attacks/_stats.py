...
class Results(object):
    ...
    def clear(self):
        ...
        self.loss = [None]*self.numSubkeys
        self.accuracy = [None]*self.numSubkeys
        self.metrics = [None]*self.numSubkeys
    ...
    def update_loss(self, bnum, data, copy=True, force_update=False):
    ...
    def update_accuracy(self, bnum, data, copy=True, force_update=False):
        
        if (id(data) != id(self.diffs[bnum])) or force_update:
            if data is not None and copy:
                self.accuracy[bnum] = data[:]
            else:
                self.accuracy[bnum] = data
    
    def update_metrics(self, bnum, data, copy=True, force_update=False):
        
        if (id(data) != id(self.diffs[bnum])) or force_update:
            if data is not None and copy:
                self.metrics[bnum] = data[:]
            else:
                self.metrics[bnum] = data
