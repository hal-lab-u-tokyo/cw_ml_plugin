...
class DAE_FIR():
    ...
    def _set_cut_off(self, cutoff:float):
        if not isinstance(cutoff, float):
            raise TypeError(f'expected float; got {type(cutoff)}')
        else:
            self.parameters._cutoff = cutoff
    
    def _set_sampling(self, sampling:float):
        if not isinstance(sampling, float):
            raise TypeError(f'expected float; got {type(sampling)}')
        else:
            self.parameters._sampling = sampling
    
    @property
    def sampling(self):
        return self.parameters._sampling
    
    @sampling.setter
    def sampling(self, sampling:float):
        self._set_sampling(sampling)
    
    @property
    def cutoff(self):
        return self.parameters._cutoff
    
    @cutoff.setter
    def cutoff(self, cutoff:float):
        self._set_cut_off(cutoff)
        
    def _set_numtaps(self, numtaps:int):
        if not isinstance(numtaps, int):
            raise TypeError(f'expected int; got {type(numtaps)}')
        else:
            self.parameters._numtaps = numtaps
    
    @property
    def numtaps(self):
        return self.parameters._numtaps
    
    @numtaps.setter
    def numtaps(self, numtaps:int):
        self._set_numtaps(numtaps)
    ...
    def preprocess(self):
        ...
        print("NORMALIZING")
        norm = Normalize(convolve_proj, minus_avg = False)
        norm_proj = norm.preprocess()
        
        if self.noize_range != None:
            print("ADDING NOIZE")
            add_noize = AddNoize(norm_proj)
            add_noize.noize_range = self.noize_range
            addnoize_proj = add_noize.preprocess()
        else:
            addnoize_proj = norm_proj
        
        print("FIR FILTERING")
        #this part will be added to the preprocess function later
        target_proj = Project()
        for t in norm_proj.traces:
            w = t.wave
            filter = firwin(numtaps=self.numtaps, cutoff=self.cutoff, fs=self.sampling)
            w = lfilter(filter, 1, w)
            target_proj.traces.append(cw.Trace(w, t.textin, t.textout, t.key)) # type: ignore
        
        print("SLICING INPUT TRACES")
        slice = Slice(addnoize_proj)
        ...
        plt.plot(input_proj.waves[0], label = "Input")
        plt.plot(target_proj.waves[0], label = "Target")
        plt.title(f"Input and Target Traces")
...
class parameters():
    def __init__(self):
        ...
        self._sampling: float = 1e9
        self._cutoff: float = 200e6
        self._numtaps: int = 200